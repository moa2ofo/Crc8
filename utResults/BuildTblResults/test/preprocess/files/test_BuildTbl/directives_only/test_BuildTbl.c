...
/* Reference CRC-8 table values (pre-calculated for verification with polynomial 0x07u) */
...
void test_BuildTbl_EntireTableCorrect(void) {
  ...
  for(uint16_t i = 0u; i < 256u; i++) { TEST_ASSERT_EQUAL_HEX8_MESSAGE(expected_crc8_table[i], Crc8Tbl_u8[i], "Table mismatch at index"); }
}
...
void test_BuildTbl_AlgorithmPattern(void) {
  ...
  /* First 16 values should be deterministic based on algorithm with polynomial 0x07u */
  uint8_t expectedFirst16[] = {0x00, 0x07, 0x0E, 0x09, 0x1C, 0x1B, 0x12, 0x15, 0x38, 0x3F, 0x36, 0x31, 0x24, 0x23, 0x2A, 0x2D};

  for(uint8_t i = 0u; i < 16u; i++) { TEST_ASSERT_EQUAL_HEX8(expectedFirst16[i], Crc8Tbl_u8[i]); }
}
