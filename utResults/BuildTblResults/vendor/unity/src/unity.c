...
void UnityPrintNumberUnsigned(const UNITY_UINT number) {
  ...
  while(number / divisor > 9) { divisor *= 10; }
...
void UnityPrintNumberHex(const UNITY_UINT number, const char nibbles_to_print) {
  ...
  if((unsigned)nibbles > UNITY_MAX_NIBBLES) { nibbles = UNITY_MAX_NIBBLES; }
...
void UnityPrintFloat(const UNITY_DOUBLE input_number) {
  ...
  if(number == 0.0f) {
    ...
    if(exponent != 0) {
      ...
      while(digits > 0) { UNITY_OUTPUT_CHAR(buf[--digits]); }
    }
  }
}
...
void UnityAssertEqualIntArray(UNITY_INTERNAL_PTR expected, UNITY_INTERNAL_PTR actual, const UNITY_UINT32 num_elements, const char *msg, const UNITY_LINE_TYPE lineNumber,
  ...
  while((elements > 0) && (elements--)) {
    ...
    if(flags == UNITY_ARRAY_TO_ARRAY) { expected = (UNITY_INTERNAL_PTR)((const char *)expected + increment); }
...
void UnityAssertWithinFloatArray(const UNITY_FLOAT delta, UNITY_PTR_ATTRIBUTE const UNITY_FLOAT *expected, UNITY_PTR_ATTRIBUTE const UNITY_FLOAT *actual, const UNITY_UINT32 num_elements,
  ...
  if(UNITY_IS_INF(in_delta)) { return; /* Arrays will be force equal with infinite delta */ }
  ...
  if(UnityIsOneArrayNull((UNITY_INTERNAL_PTR)expected, (UNITY_INTERNAL_PTR)actual, lineNumber, msg)) { UNITY_FAIL_AND_BAIL; }
  ...
  while(elements--) {
    ...
    if(flags == UNITY_ARRAY_TO_ARRAY) { ptr_expected++; }
    ptr_actual++;
  }
}
...
void UnityAssertFloatSpecial(const UNITY_FLOAT actual, const char *msg, const UNITY_LINE_TYPE lineNumber, const UNITY_FLOAT_TRAIT_T style) {
  ...
  if(is_trait != should_be_trait) {
    UnityTestResultsFailBegin(lineNumber);
    UnityPrint(UnityStrExpected);
    if(!should_be_trait) { UnityPrint(UnityStrNot); }
...
void UnityAssertEqualString(const char *expected, const char *actual, const char *msg, const UNITY_LINE_TYPE lineNumber) {
  ...
  if(expected && actual) {
    ...
    if(expected || actual) { Unity.CurrentTestFailed = 1; }
  }
...
void UnityAssertEqualStringArray(UNITY_INTERNAL_PTR expected, const char **actual, const UNITY_UINT32 num_elements, const char *msg, const UNITY_LINE_TYPE lineNumber, const UNITY_FLAGS_T flags) {
  ...
  if((const void *)expected == (const void *)actual) { return; /* Both are NULL or same pointer */ }

  if(UnityIsOneArrayNull((UNITY_INTERNAL_PTR)expected, (UNITY_INTERNAL_PTR)actual, lineNumber, msg)) { UNITY_FAIL_AND_BAIL; }

  if(flags != UNITY_ARRAY_TO_ARRAY) { expd = (const char *)expected; }

  do {
    act = actual[j];
    if(flags == UNITY_ARRAY_TO_ARRAY) { expd = ((const char *const *)expected)[j]; }
    ...
    if(expd && act) {
      ...
      if(expd != act) { Unity.CurrentTestFailed = 1; }
    }
...
void UnityAssertEqualMemory(UNITY_INTERNAL_PTR expected, UNITY_INTERNAL_PTR actual, const UNITY_UINT32 length, const UNITY_UINT32 num_elements, const char *msg, const UNITY_LINE_TYPE lineNumber,
  ...
  if(length == 0) { UnityPrintPointlessAndBail(); }
  ...
  while(elements--) {
    ...
    if(flags == UNITY_ARRAY_TO_VAL) { ptr_exp = (UNITY_PTR_ATTRIBUTE const unsigned char *)expected; }
  }
}
...
static int IsStringInBiggerString(const char *longstring, const char *shortstring) {
  ...
  if(*sptr == '*') { return UnityStrictMatch ? 0 : 1; }
...
static int UnityStringArgumentMatches(const char *str) {
  ...
  while(ptr1[0] != 0) {
    ...
    do {
      ptr2++;
      if((ptr2[0] == ':') && (ptr2[1] != 0) && (ptr2[0] != '\'') && (ptr2[0] != '"') && (ptr2[0] != ',')) { ptrf = &ptr2[1]; }
    ...
    if((retval == 2) && (ptrf != 0)) {
      if(IsStringInBiggerString(Unity.CurrentTestName, ptrf)) { return 1; }
    }

    /* done if complete testname match */
    if(IsStringInBiggerString(Unity.CurrentTestName, ptr1) == 1) { return 1; }
...
int UnityTestMatches(void) {
  ...
  if(UnityOptionExcludeNamed) {
    if(UnityStringArgumentMatches(UnityOptionExcludeNamed)) { retval = 0; }
  }
