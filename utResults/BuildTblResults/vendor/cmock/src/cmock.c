...
CMOCK_MEM_INDEX_TYPE CMock_Guts_MemNew(CMOCK_MEM_INDEX_TYPE size) {
  ...
  if(size < 1) { return CMOCK_GUTS_NONE; }

  /* verify we have enough room */
  size = size + CMOCK_MEM_INDEX_SIZE;
  if(size & CMOCK_MEM_ALIGN_MASK) { size = (size + CMOCK_MEM_ALIGN_MASK) & ~CMOCK_MEM_ALIGN_MASK; }
  if((CMock_Guts_BufferSize - CMock_Guts_FreePtr) < size) {
    ...
    if(new_buffer == NULL) return CMOCK_GUTS_NONE; /* realloc() failed; out of memory */
...
CMOCK_MEM_INDEX_TYPE CMock_Guts_MemChain(CMOCK_MEM_INDEX_TYPE root_index, CMOCK_MEM_INDEX_TYPE obj_index) {
  ...
  if(root_index == CMOCK_GUTS_NONE) {
    ...
    if((root_index < CMOCK_MEM_ALIGN_SIZE) || (root_index >= CMock_Guts_FreePtr)) { return CMOCK_GUTS_NONE; }
    if((obj_index < CMOCK_MEM_ALIGN_SIZE) || (obj_index >= CMock_Guts_FreePtr)) { return CMOCK_GUTS_NONE; }
    ...
    do {
      ...
      if(index >= CMock_Guts_FreePtr) { return CMOCK_GUTS_NONE; }
      if(index > 0) { next = (void *)(&CMock_Guts_Buffer[index]); }
    } while(index > 0);
...
CMOCK_MEM_INDEX_TYPE CMock_Guts_MemNext(CMOCK_MEM_INDEX_TYPE previous_item_index) {
  ...
  if((previous_item_index < CMOCK_MEM_ALIGN_SIZE) || (previous_item_index >= CMock_Guts_FreePtr)) { return CMOCK_GUTS_NONE; }
...
CMOCK_MEM_INDEX_TYPE CMock_Guts_MemEndOfChain(CMOCK_MEM_INDEX_TYPE root_index) {
  ...
  for(next_index = root_index; next_index != CMOCK_GUTS_NONE; next_index = CMock_Guts_MemNext(index)) { index = next_index; }
